...
from app.components.styling import render_info_banner, render_metric_card
...
def render_badge(label: str, css_class: str, helper_text: str) -> None:
    st.markdown(
        f"<div class='{css_class}'>{label}<br><span style='font-size:0.9rem; font-weight:500;'>{helper_text}</span></div>",
...
def render() -> None:
    ...
    with col1:
        ...
        amh_ng_ml = st.number_input("AMH (ng/mL)", 0.1, 20.0, 4.2, 0.1, help="Anti-Mullerian Hormone. Values above 3.5 ng/mL are associated with PCOS.")
        ...
        st.markdown(
            f"<div class='section-card'><strong>Live FSH/LH ratio</strong><br><span style='font-size:1.8rem; color:#2176AE; font-weight:700;'>{live_ratio:.2f}</span></div>",
            unsafe_allow_html=True,
        )
    ...
    if st.button("Run Clinical Benchmark Prediction", key="run_invasive"):
