...
from app.components.styling import render_info_banner, render_section_card
...
def render() -> None:
    ...
    age_summary = (
        clinical_df.groupby("pcos_y_n")["age_yrs"]
        .agg(["mean", "median", "std"])
        .round(2)
        .rename(index={0: "No PCOS", 1: "PCOS"})
    )
...
"""
        ...
        symptom_gap_display = symptom_gap.rename(columns={0: "No PCOS %", 1: "PCOS %", "gap": "Gap %"})
        st.dataframe(symptom_gap_display, use_container_width=True)
        top_symptoms = [name.replace("_y_n", "").replace("_", " ") for name in symptom_gap_display.index[:3]]
        st.markdown(
            f"""
**Symptom insight:** The strongest visible symptom gaps are coming from {top_symptoms[0]}, {top_symptoms[1]}, and {top_symptoms[2]}.
Skin darkening alone is separating the groups by about {symptom_gap_display.iloc[0]['Gap %']:.1f} percentage points, which is a very strong screening clue.
...
"""
        ...
        render_section_card(
            ...
            The most striking symptom gaps are skin darkening ({symptom_gap_display.iloc[0]['Gap %']:.1f} percentage points), weight gain ({symptom_gap_display.loc['weight_gain_y_n', 'Gap %']:.1f} percentage points), and hair growth ({symptom_gap_display.loc['hair_growth_y_n', 'Gap %']:.1f} percentage points).
...
This is a large shift and it fits the clinical literature where elevated AMH is often linked with PCOS.
"""
        )
