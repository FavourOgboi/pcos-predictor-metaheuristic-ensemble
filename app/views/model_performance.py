...
def render() -> None:
    ...
    try:
        ...
        st.error(f"Unable to load the final model results: {exc}")
        return
    ...
    with st.expander("Individual Models (Logistic Regression, Random Forest, XGBoost)", expanded=False):
        st.markdown(
            "Logistic Regression is acting as the simple linear baseline. Random Forest is capturing non-linear splits and interactions. XGBoost is adding boosted tree learning that often performs strongly on tabular clinical data."
        )
    ...
    with st.expander("Metaheuristic Optimization (WaO, RSO, CSO)", expanded=False):
        st.markdown(
            "Metaheuristic optimization is tuning the stack hyperparameters with nature-inspired search. WaO, RSO, and CSO are exploring different parts of the search space, keeping good candidates, and improving the final ensemble beyond the baseline stacking stage."
        )
    ...
    for tab, model_set in zip(tabs[:2], ["Model 1", "Model 2"]):
        with tab:
            ...
            styled = (
                subset[["model", "approach", "algorithm"] + numeric_cols]
                .style.highlight_max(subset=numeric_cols, color="#D1FAE5")
                .highlight_min(subset=numeric_cols, color="#FEE2E2")
    ...
    with col1:
        ...
        st.markdown(
            "This chart is showing how WaO, RSO, and CSO searched the Model 1 non-invasive space. It helps show which optimizer found a strong answer fastest."
        )
    with col2:
        show_image_or_placeholder(IMAGE_PATHS["convergence_m2"], "Model 2 optimization convergence")
        st.markdown(
            "This chart is showing the same optimization story for the invasive benchmark model."
        )

    show_image_or_placeholder(IMAGE_PATHS["master_heatmap"], "Master optimization heatmap")
    st.markdown(
        "The master heatmap is summarizing all phases together - individual models, stacking, and optimized ensembles. It gives one compact view of which approach performed best on each metric."
    )

    gap = best_rows["Model 2"]["auc"] - best_rows["Model 1"]["auc"]
    individual_gap = best_individual["Model 2"]["auc"] - best_individual["Model 1"]["auc"]
    render_section_card(
        ...
        The current gap is {gap:+.3f} percentage points, compared with an individual-model gap of {individual_gap:+.3f}.
        In this repo state the non-invasive model is matching or slightly exceeding the invasive benchmark, which strongly supports practical PCOS screening without laboratory tests.
        """,
    )
    ...
    with shap_cols[0]:
        ...
        st.markdown(
            "The non-invasive model is being driven by routine screening signals such as BMI, cycle pattern, symptom burden, and metabolic markers."
        )
    with shap_cols[1]:
        show_image_or_placeholder(IMAGE_PATHS["shap_m2_bar"], "Invasive Benchmark Feature Importance")
        st.markdown(
            "The invasive model is being driven by hormonal and ultrasound features such as AMH, follicle burden, and endocrine ratios."
        )
